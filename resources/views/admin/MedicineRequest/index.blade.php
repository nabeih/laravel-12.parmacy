...
<div class="admin-page-header">
    <div>
        <h1 class="admin-page-title">طلبات الأدوية</h1>
        <p class="admin-page-sub">طلبات الصيادلة لإضافة أدوية جديدة إلى الكتالوج الرئيسي</p>
    </div>
    <div style="display:flex;gap:8px">
        <a href="{{ route('admin.medicine_request.index') }}"
            class="admin-btn {{ !$status ? 'admin-btn-primary' : 'admin-btn-outline' }}">الكل</a>
        <a href="{{ route('admin.medicine_request.index', ['status' => 'pending']) }}"
            class="admin-btn {{ $status == 'pending' ? 'admin-btn-primary' : 'admin-btn-outline' }}">قيد المراجعة</a>
        <a href="{{ route('admin.medicine_request.index', ['status' => 'approved']) }}"
            class="admin-btn {{ $status == 'approved' ? 'admin-btn-primary' : 'admin-btn-outline' }}">معتمدة</a>
        <a href="{{ route('admin.medicine_request.index', ['status' => 'rejected']) }}"
                    ...
                    <tr>
                        ...
                        <td>
                            @if($req->status == 'pending')
                                <a href="{{ route('admin.medicine_request.review', $req->id) }}"
                                    class="admin-btn admin-btn-sm admin-btn-outline">مراجعة</a>
                                <form action="{{ route('admin.medicine_request.reject', $req->id) }}" method="POST"
                    ...
                    <tr>
                        <td colspan="6">
                            <div class="admin-empty">
                                ...
                                <div class="admin-empty-title">لا يوجد طلبات أدوية</div>
                            </div>
                        </td>
                    </tr>
                @endforelse
