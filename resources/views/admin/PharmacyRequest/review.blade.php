...
<div class="admin-card">
    ...
    @if($pharmacyRequest->target_pharmacy_id)
        {{-- Join-existing-vacant: the pharmacy already exists, just confirm the reassignment. --}}
        <div class="admin-section-title">✅ الموافقة على الانضمام</div>
        <p class="admin-page-sub">
            طلب انضمام إلى صيدلية قائمة: <strong>{{ $pharmacyRequest->targetPharmacy->name_ar ?? '-' }}</strong>
            ({{ $pharmacyRequest->targetPharmacy->name_en ?? '-' }}) — {{ $pharmacyRequest->targetPharmacy->address ?? '-' }}
        </p>
        <form action="{{ route('admin.pharmacy_request.approve', $pharmacyRequest->id) }}" method="POST">
            ...
            <div class="admin-modal-footer" style="border-top:none;padding-top:0">
                <button type="submit" class="admin-btn admin-btn-primary">✅ الموافقة على الانضمام</button>
            </div>
        </form>
    @else
